<?php include 'db.php';?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
	<title>registration</title>
	<link rel="stylesheet" type="text/css" href="style.css">
</head>
<body>
	<header>
   <div class="container">
   	<div class="logo">
   	 		<a href="index.php"><img src="images/house13.png"><span>HRMS</span></a>
   	</div>
   </div>
</header>
<div class="home">	
<div class="left-sidebar">
  		<input type="text" class="searchbox" name="search" placeholder="Search..." data-toggle="tooltip" title="search here!" autocomplete="off">
  		<button class="search">Search</button>
  		<ul>
  			<li><a href="index.php">መግቢያ</a></li>
  			<li><a href="rentersregistration.php">አዲስ ተከራይ ማስገቢያ</a></li>
  			<li><a href="renterlist.php">ተከራይ ዝርዝር </a></li>
  			<li><a href="payment.php">የክፍያ መሰብሰቢያ</a></li>
  			<li><a href="paymentlist.php">የክፍያ መከታተያ</a></li>
  			<li><a href="logout.php">Logout</a></li>
  		</ul>
</div>
<div class="main-content">
	<h2>አዲስ ተከራይ ማስገቢያ</h2>
	<a href="renterlist.php">ወደ ዝርዝር ለመመለስ</a>
<form action="rentersregistration.php" method="post" enctype="multipart/form-data">
	<!-- display validation errors-->
    <?php include('errors.php'); ?>
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <div class="all3">
	<div class="renterinfo">
	<label>ሙሉ ስም</label><br>
	<input type="text" name="name" class="textinput" autocomplete="off">
	</div>

	<div class="renterinfo">
	<label>ጾታ አይነት</label><br>
	<select name="sex" class="textinput">
     <option>Select your sex</option>
     <option >Male</option>
     <option >Female</option>
    </select>
    </div>

    <div class="renterinfo">
	<label>የምዝገባ ቀን</label><br>
	<input type="date" name="date" class="textinput" autocomplete="off">
	</div>
    </div>
    <div class="all3">
	<div class="renterinfo">
	<label>የቤት አይነት </label><br>
	<select name="room" class="textinput">
     <option>Select house type</option>
     <option >One Room</option>
     <option >Two Room</option>
    </select>
    </div>

	<div class="renterinfo">
	<label>ክልል</label><br>
	<select name="kilil" class="textinput">
     <option>Select your region</option>
     <option >South</option>
     <option >Oromiya</option>
     <option >Amhara</option>
     <option >Tigray</option>
     <option >Somali</option>
     <option >Harari</option>
     <option >Afar</option>
     <option >Gambela</option>
     <option >Benishangul</option>
     <option >Oromiya</option>
     <option >Addis Ababa</option>
     <option >Diredawa</option>
     <option >Sidama</option>
  </select>
    </div>

	<div class="renterinfo">
	<label>ከተማ</label><br>
	<input type="text" name="city" class="textinput" autocomplete="off">
	</div>
    </div>

    <div class="all3">
	<div class="renterinfo">
	<label>ወረዳ</label><br>
	<input type="text" name="woreda" class="textinput" autocomplete="off">
	</div>

	<div class="renterinfo">
	<label>ቀበሌ</label><br>
	<input type="text" name="kebele" class="textinput" autocomplete="off">
	</div>

	<div class="renterinfo">
	<label>ሰልክ ቁጥር</label><br>
	<input type="text" name="number" class="textinput" autocomplete="off">
	</div>
    </div>

	<div class="renterinfo">
		<?php if ($edit_state == false): ?>
	<input type="submit" name="photo" value="መዝገብ/Register" class="textinput">
	    <?php else: ?>
	<input type="submit" name="update" value="መዝገብ/update" class="textinput">
	    <?php endif ?>   	
	</div>
</form>   
</div>
</div>
<div class="footer">
  <p>&copy;Copyright 2020 |Designed by Kasahun Abebe</p>
</div>
</body>
</html>